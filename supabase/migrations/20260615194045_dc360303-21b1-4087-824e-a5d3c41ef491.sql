...
ALTER TABLE public.barbearias ADD COLUMN IF NOT EXISTS deleted_at timestamptz;

-- 2) Soft delete + restore
CREATE OR REPLACE FUNCTION public.soft_delete_barbearia(
...
BEGIN
  ...
  END IF;
  UPDATE public.barbearias SET deleted_at = now(), updated_at = now()
  WHERE id = p_id AND deleted_at IS NULL;
  RETURN FOUND;
END $$;

CREATE OR REPLACE FUNCTION public.restore_barbearia(
  p_admin_id uuid, p_admin_login text, p_admin_senha text, p_id uuid
) RETURNS boolean LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM public.usuarios
    ...
    RAISE EXCEPTION 'Acesso não autorizado.';
  END IF;
  UPDATE public.barbearias SET deleted_at = NULL, updated_at = now()
  WHERE id = p_id AND deleted_at IS NOT NULL;
  RETURN FOUND;
END $$;

-- 3) Filtrar arquivadas em list_clubes_publicos
...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT c.id, c.nome, c.valor_mensal, c.descricao
  FROM public.clube_assinatura c
  JOIN public.barbearias b ON b.id = c.barbearia_id
  WHERE c.barbearia_id = p_barbearia_id
    AND c.ativo = true AND c.deleted_at IS NULL
    AND b.deleted_at IS NULL
  ORDER BY c.valor_mensal ASC;
$$;

-- 4) Rollback (hard delete) de uma barbearia recém-criada
...
-- 5) clone_barbearia — clona dados de uma barbearia modelo para uma nova
...
BEGIN
  ...
  IF p_new_slug !~ '^[a-z0-9-]{3,30}$' THEN
    RAISE EXCEPTION 'Slug inválido. Use 3-30 caracteres: a-z, 0-9 e hífen.';
  END IF;
  IF p_new_slug = ANY(v_reserved) THEN
    RAISE EXCEPTION 'Este slug é reservado.';
  END IF;
  IF EXISTS (SELECT 1 FROM public.barbearias WHERE slug = p_new_slug) THEN
    RAISE EXCEPTION 'Já existe uma barbearia com este slug.';
  END IF;
  IF char_length(btrim(coalesce(p_new_nome,''))) < 1 THEN
    RAISE EXCEPTION 'Informe o nome da barbearia.';
  END IF;
  IF char_length(btrim(coalesce(p_new_admin_login,''))) < 3 THEN
    RAISE EXCEPTION 'Login do admin inválido.';
  END IF;
  IF char_length(coalesce(p_new_admin_senha,'')) < 6 THEN
    RAISE EXCEPTION 'Senha do admin deve ter no mínimo 6 caracteres.';
  END IF;
  ...
  -- usuarios (sem o admin nível 0, que não pertence à barbearia modelo)
  ...
  -- Atualiza admin nivel 1 com login/senha fornecidos (assume um por barbearia)
  ...
  -- colaborador_servicos (reescreve FKs)
  ...
  -- blog (autor_id pode referenciar usuarios da barbearia ou ser global - tenta remapear)
  ...
  -- clube_assinatura (reescreve servico_id em regras_servicos)
  ...
  INSERT INTO public.cupons_desconto (id, barbearia_id, nome, descricao, codigo,
    ...
    regras_servicos, deleted_at, created_at, updated_at)
  SELECT m.new_id, v_new_id, c.nome, c.descricao, c.codigo, c.data_inicio, c.data_fim,
    c.dias_semana, c.limite_por_cliente, c.somente_novos_clientes, c.inatividade_dias,
    c.valor_minimo_total, c.tipo_desconto_total, c.valor_desconto_total,
    (SELECT COALESCE(jsonb_agg(
      regra || jsonb_build_object('servico_id',
        COALESCE((SELECT new_id FROM _map_servicos WHERE old_id = (regra->>'servico_id')::uuid),
                 (regra->>'servico_id')::uuid))
    ...
    FROM jsonb_array_elements(c.regras_servicos) regra),
    c.deleted_at, c.created_at, c.updated_at
  FROM public.cupons_desconto c JOIN _map_cupons m ON m.old_id = c.id
  WHERE c.barbearia_id = p_modelo_id;

  -- atendimentos
  INSERT INTO public.atendimentos (id, cliente_id, colaborador_id, data, valor, status,
    created_at, updated_at, comissao, servicos_atendimento, pedido_exclusao, barbearia_id,
    cupom_id, cupom_codigo, cupom_nome, cupom_status, cupom_motivo_invalidacao,
    valor_original, valor_desconto, cupom_aplicado_em, cupom_invalidado_em, clube_id,
    clube_desconto_aplicado)
  SELECT m.new_id,
    (SELECT new_id FROM _map_usuarios WHERE old_id = a.cliente_id),
    (SELECT new_id FROM _map_colaboradores WHERE old_id = a.colaborador_id),
    a.data, a.valor, a.status, a.created_at, a.updated_at, a.comissao, a.servicos_atendimento,
    a.pedido_exclusao, v_new_id,
    (SELECT new_id FROM _map_cupons WHERE old_id = a.cupom_id),
    a.cupom_codigo, a.cupom_nome, a.cupom_status, a.cupom_motivo_invalidacao,
    a.valor_original, a.valor_desconto, a.cupom_aplicado_em, a.cupom_invalidado_em,
    (SELECT new_id FROM _map_clubes WHERE old_id = a.clube_id),
    a.clube_desconto_aplicado
  FROM public.atendimentos a JOIN _map_atendimentos m ON m.old_id = a.id
  WHERE a.barbearia_id = p_modelo_id;

  -- atendimento_servicos
  INSERT INTO public.atendimento_servicos (atendimento_id, servico_id, valor_servico,
    created_at, name_servico, barbearia_id, valor_original, valor_desconto,
    tipo_desconto_cupom, valor_regra_cupom)
  SELECT ma.new_id, ms.new_id, asv.valor_servico, asv.created_at, asv.name_servico,
    v_new_id, asv.valor_original, asv.valor_desconto, asv.tipo_desconto_cupom,
    asv.valor_regra_cupom
  FROM public.atendimento_servicos asv
  JOIN _map_atendimentos ma ON ma.old_id = asv.atendimento_id
  JOIN _map_servicos ms ON ms.old_id = asv.servico_id
  WHERE asv.barbearia_id = p_modelo_id;

  -- gastos
  INSERT INTO public.gastos (nome, valor, data, created_at, barbearia_id)
  SELECT nome, valor, data, created_at, v_new_id
  FROM public.gastos WHERE barbearia_id = p_modelo_id;

  -- transacoes_financeiras
  INSERT INTO public.transacoes_financeiras (tipo, categoria, descricao, valor, data,
    referencia_id, created_at, updated_at, barbearia_id)
  SELECT tipo, categoria, descricao, valor, data,
    COALESCE((SELECT new_id FROM _map_atendimentos WHERE old_id = referencia_id), referencia_id),
    created_at, updated_at, v_new_id
  FROM public.transacoes_financeiras WHERE barbearia_id = p_modelo_id;

  -- ============ Rewrite URLs (substituir UUID antigo pelo novo no path do storage) ============
  -- Substitui barbearia_id segment em todas as colunas de imagem da nova barbearia
  PERFORM 1;

  -- servicos
  UPDATE public.servicos s SET
    image_url   = replace(replace(s.image_url,   '/'||p_modelo_id::text||'/', '/'||v_new_id::text||'/'),
                    '/'||(SELECT old_id FROM _map_servicos WHERE new_id = s.id)::text||'/',
                    '/'||s.id::text||'/'),
    image_url_2 = replace(replace(s.image_url_2, '/'||p_modelo_id::text||'/', '/'||v_new_id::text||'/'),
                    '/'||(SELECT old_id FROM _map_servicos WHERE new_id = s.id)::text||'/',
                    '/'||s.id::text||'/'),
    image_url_3 = replace(replace(s.image_url_3, '/'||p_modelo_id::text||'/', '/'||v_new_id::text||'/'),
                    '/'||(SELECT old_id FROM _map_servicos WHERE new_id = s.id)::text||'/',
                    '/'||s.id::text||'/'),
    image_url_4 = replace(replace(s.image_url_4, '/'||p_modelo_id::text||'/', '/'||v_new_id::text||'/'),
                    '/'||(SELECT old_id FROM _map_servicos WHERE new_id = s.id)::text||'/',
                    '/'||s.id::text||'/'),
    image_url_5 = replace(replace(s.image_url_5, '/'||p_modelo_id::text||'/', '/'||v_new_id::text||'/'),
                    '/'||(SELECT old_id FROM _map_servicos WHERE new_id = s.id)::text||'/',
                    '/'||s.id::text||'/')
  WHERE s.barbearia_id = v_new_id;

  -- colaboradores
  UPDATE public.colaboradores c SET
    foto_url   = replace(replace(c.foto_url,   '/'||p_modelo_id::text||'/', '/'||v_new_id::text||'/'),
                   '/'||(SELECT old_id FROM _map_colaboradores WHERE new_id = c.id)::text||'/',
                   '/'||c.id::text||'/'),
    foto_url_2 = replace(replace(c.foto_url_2, '/'||p_modelo_id::text||'/', '/'||v_new_id::text||'/'),
                   '/'||(SELECT old_id FROM _map_colaboradores WHERE new_id = c.id)::text||'/',
                   '/'||c.id::text||'/'),
    foto_url_3 = replace(replace(c.foto_url_3, '/'||p_modelo_id::text||'/', '/'||v_new_id::text||'/'),
                   '/'||(SELECT old_id FROM _map_colaboradores WHERE new_id = c.id)::text||'/',
                   '/'||c.id::text||'/'),
    foto_url_4 = replace(replace(c.foto_url_4, '/'||p_modelo_id::text||'/', '/'||v_new_id::text||'/'),
                   '/'||(SELECT old_id FROM _map_colaboradores WHERE new_id = c.id)::text||'/',
                   '/'||c.id::text||'/'),
    foto_url_5 = replace(replace(c.foto_url_5, '/'||p_modelo_id::text||'/', '/'||v_new_id::text||'/'),
                   '/'||(SELECT old_id FROM _map_colaboradores WHERE new_id = c.id)::text||'/',
                   '/'||c.id::text||'/'),
    foto_url_6 = replace(replace(c.foto_url_6, '/'||p_modelo_id::text||'/', '/'||v_new_id::text||'/'),
                   '/'||(SELECT old_id FROM _map_colaboradores WHERE new_id = c.id)::text||'/',
                   '/'||c.id::text||'/'),
    foto_url_7 = replace(replace(c.foto_url_7, '/'||p_modelo_id::text||'/', '/'||v_new_id::text||'/'),
                   '/'||(SELECT old_id FROM _map_colaboradores WHERE new_id = c.id)::text||'/',
                   '/'||c.id::text||'/')
  WHERE c.barbearia_id = v_new_id;

  -- informacoes
  UPDATE public.informacoes i SET
    imagem_1 = replace(replace(i.imagem_1, '/'||p_modelo_id::text||'/', '/'||v_new_id::text||'/'),
                 '/'||(SELECT old_id FROM _map_informacoes WHERE new_id = i.id)::text||'/',
                 '/'||i.id::text||'/'),
    imagem_2 = replace(replace(i.imagem_2, '/'||p_modelo_id::text||'/', '/'||v_new_id::text||'/'),
                 '/'||(SELECT old_id FROM _map_informacoes WHERE new_id = i.id)::text||'/',
                 '/'||i.id::text||'/'),
    imagem_3 = replace(replace(i.imagem_3, '/'||p_modelo_id::text||'/', '/'||v_new_id::text||'/'),
                 '/'||(SELECT old_id FROM _map_informacoes WHERE new_id = i.id)::text||'/',
                 '/'||i.id::text||'/'),
    imagem_4 = replace(replace(i.imagem_4, '/'||p_modelo_id::text||'/', '/'||v_new_id::text||'/'),
                 '/'||(SELECT old_id FROM _map_informacoes WHERE new_id = i.id)::text||'/',
                 '/'||i.id::text||'/'),
    imagem_5 = replace(replace(i.imagem_5, '/'||p_modelo_id::text||'/', '/'||v_new_id::text||'/'),
                 '/'||(SELECT old_id FROM _map_informacoes WHERE new_id = i.id)::text||'/',
                 '/'||i.id::text||'/'),
    imagem_6 = replace(replace(i.imagem_6, '/'||p_modelo_id::text||'/', '/'||v_new_id::text||'/'),
                 '/'||(SELECT old_id FROM _map_informacoes WHERE new_id = i.id)::text||'/',
                 '/'||i.id::text||'/'),
    imagem_7 = replace(replace(i.imagem_7, '/'||p_modelo_id::text||'/', '/'||v_new_id::text||'/'),
                 '/'||(SELECT old_id FROM _map_informacoes WHERE new_id = i.id)::text||'/',
                 '/'||i.id::text||'/'),
    imagem_8 = replace(replace(i.imagem_8, '/'||p_modelo_id::text||'/', '/'||v_new_id::text||'/'),
                 '/'||(SELECT old_id FROM _map_informacoes WHERE new_id = i.id)::text||'/',
                 '/'||i.id::text||'/'),
    imagem_logo = replace(replace(i.imagem_logo, '/'||p_modelo_id::text||'/', '/'||v_new_id::text||'/'),
                 '/'||(SELECT old_id FROM _map_informacoes WHERE new_id = i.id)::text||'/',
                 '/'||i.id::text||'/'),
    foto_perfil = replace(replace(i.foto_perfil, '/'||p_modelo_id::text||'/', '/'||v_new_id::text||'/'),
                 '/'||(SELECT old_id FROM _map_informacoes WHERE new_id = i.id)::text||'/',
                 '/'||i.id::text||'/')
  WHERE i.barbearia_id = v_new_id;

  -- promocao
  UPDATE public.promocao p SET
    imagem_promo  = replace(replace(p.imagem_promo,  '/'||p_modelo_id::text||'/', '/'||v_new_id::text||'/'),
                      '/'||(SELECT old_id FROM _map_promocao WHERE new_id = p.id)::text||'/',
                      '/'||p.id::text||'/'),
    imagem_ia     = replace(replace(p.imagem_ia,     '/'||p_modelo_id::text||'/', '/'||v_new_id::text||'/'),
                      '/'||(SELECT old_id FROM _map_promocao WHERE new_id = p.id)::text||'/',
                      '/'||p.id::text||'/'),
    imagem_banner = replace(replace(p.imagem_banner, '/'||p_modelo_id::text||'/', '/'||v_new_id::text||'/'),
                      '/'||(SELECT old_id FROM _map_promocao WHERE new_id = p.id)::text||'/',
                      '/'||p.id::text||'/')
  WHERE p.barbearia_id = v_new_id;

  -- blog
  UPDATE public.blog b SET
    imagem_url = replace(replace(b.imagem_url, '/'||p_modelo_id::text||'/', '/'||v_new_id::text||'/'),
                   '/'||(SELECT old_id FROM _map_blog WHERE new_id = b.id)::text||'/',
                   '/'||b.id::text||'/')
  WHERE b.barbearia_id = v_new_id;

  -- agentes_ia: apenas barbearia segment (estrutura interna do agente é variável)
  UPDATE public.agentes_ia SET
    imagem_objetivo = replace(imagem_objetivo, '/'||p_modelo_id::text||'/', '/'||v_new_id::text||'/'),
    imagem_campanha = replace(imagem_campanha, '/'||p_modelo_id::text||'/', '/'||v_new_id::text||'/'),
    imagem_estilovisual = replace(imagem_estilovisual, '/'||p_modelo_id::text||'/', '/'||v_new_id::text||'/'),
    imagem_informacoes = replace(imagem_informacoes, '/'||p_modelo_id::text||'/', '/'||v_new_id::text||'/'),
    imagem_imareferencia = replace(imagem_imareferencia, '/'||p_modelo_id::text||'/', '/'||v_new_id::text||'/'),
    imagem_comlogo = replace(imagem_comlogo, '/'||p_modelo_id::text||'/', '/'||v_new_id::text||'/'),
    imagem_criada_ia = replace(imagem_criada_ia, '/'||p_modelo_id::text||'/', '/'||v_new_id::text||'/'),
    imagem_referencia_ia = replace(imagem_referencia_ia, '/'||p_modelo_id::text||'/', '/'||v_new_id::text||'/'),
    imagem_endereco = replace(imagem_endereco, '/'||p_modelo_id::text||'/', '/'||v_new_id::text||'/'),
    imagem_instagram = replace(imagem_instagram, '/'||p_modelo_id::text||'/', '/'||v_new_id::text||'/'),
    imagem_telcontato = replace(imagem_telcontato, '/'||p_modelo_id::text||'/', '/'||v_new_id::text||'/'),
    imagem_paleta = replace(imagem_paleta, '/'||p_modelo_id::text||'/', '/'||v_new_id::text||'/'),
    imagem_elem_central = replace(imagem_elem_central, '/'||p_modelo_id::text||'/', '/'||v_new_id::text||'/'),
    edit_imagemupada = replace(edit_imagemupada, '/'||p_modelo_id::text||'/', '/'||v_new_id::text||'/'),
    edit_imagemeditada = replace(edit_imagemeditada, '/'||p_modelo_id::text||'/', '/'||v_new_id::text||'/')
  WHERE barbearia_id = v_new_id;

  -- Build mapping JSON to return (parent_id mappings needed by storage clone in TS)
  ...
  RETURN jsonb_build_object('id', v_new_id, 'slug', p_new_slug, 'mapping', v_mapping);
END $$;
