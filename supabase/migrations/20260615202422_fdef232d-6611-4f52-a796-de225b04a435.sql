...
BEGIN
  ...
  v_suffix := '_' || replace(v_new_id::text, '-', '');
  ...
  -- usuarios — sufixar login para evitar conflito de UNIQUE
  ...
  SELECT m.new_id, u.nome, u.login || v_suffix, u.senha, u.nivel, u.created_at, u.updated_at,
  ...
  IF EXISTS (SELECT 1 FROM public.usuarios WHERE login = p_new_admin_login) THEN
    RAISE EXCEPTION 'Login de admin já está em uso.';
  END IF;
  ...
  SELECT m.new_id, c.nome, c.resumo,
    CASE WHEN c.login IS NULL OR c.login = '' THEN c.login ELSE c.login || v_suffix END,
