...
-- 1) Add ativa flag
ALTER TABLE public.barbearias ADD COLUMN IF NOT EXISTS ativa boolean NOT NULL DEFAULT true;

-- 2) Allow same colaborador login across barbearias
ALTER TABLE public.colaboradores DROP CONSTRAINT IF EXISTS colaboradores_login_key;
DROP INDEX IF EXISTS public.colaboradores_login_key;
CREATE UNIQUE INDEX IF NOT EXISTS colaboradores_login_barbearia_unique
  ON public.colaboradores (login, barbearia_id) WHERE login IS NOT NULL;

-- 3) Toggle ativa
CREATE OR REPLACE FUNCTION public.set_barbearia_ativa(
  p_admin_id uuid, p_admin_login text, p_admin_senha text, p_id uuid, p_ativa boolean
) RETURNS boolean
...
BEGIN
  ...
  END IF;
  UPDATE public.barbearias SET ativa = p_ativa, updated_at = now()
  WHERE id = p_id AND deleted_at IS NULL;
  RETURN FOUND;
END $$;

-- 4) Update slug
CREATE OR REPLACE FUNCTION public.update_barbearia_slug(
  p_admin_id uuid, p_admin_login text, p_admin_senha text, p_id uuid, p_new_slug text
) RETURNS boolean
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE
...
BEGIN
  ...
  END IF;
  IF EXISTS (SELECT 1 FROM public.barbearias WHERE slug = p_new_slug AND id <> p_id) THEN
    RAISE EXCEPTION 'Já existe uma barbearia com este slug.';
  END IF;
  UPDATE public.barbearias SET slug = p_new_slug, updated_at = now() WHERE id = p_id;
  RETURN FOUND;
END $$;

-- 5) Rewrite clone_barbearia: do NOT append suffix to logins
...
BEGIN
  ...
  -- usuarios — SEM sufixo (unique é por (login, barbearia_id))
  ...
  IF EXISTS (SELECT 1 FROM public.usuarios WHERE login = p_new_admin_login AND barbearia_id = v_new_id) THEN
  ...
  SELECT m.new_id, c.nome, c.resumo, c.login,
    c.senha, c.salario_fixo, c.foto_url,
