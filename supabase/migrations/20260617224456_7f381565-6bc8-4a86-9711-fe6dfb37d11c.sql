ALTER TABLE public.promocao RENAME COLUMN imagem_ia TO imagem_upada;
...
BEGIN
  ...
  UPDATE public.promocao p SET
    ...
    imagem_upada  = _clone_rewrite_url(imagem_upada,  p_old_barb, p_new_barb, m.old_id, p.id),
...
BEGIN
  ...
  INSERT INTO public.usuarios (id, nome, login, senha, nivel, created_at, updated_at,
    observacao, registro, promocao, recovery_token, barbearia_id)
  ...
  INSERT INTO public.clube_usuarios (barbearia_id, usuario_id, clube_id,
    data_inicio, data_fim, valor_pago, status, cancelada_em, motivo_cancelamento, created_at)
  SELECT v_new_id,
    (SELECT new_id FROM _map_usuarios WHERE old_id = cu.usuario_id),
    (SELECT new_id FROM _map_clubes WHERE old_id = cu.clube_id),
    cu.data_inicio, cu.data_fim, cu.valor_pago, cu.status,
    cu.cancelada_em, cu.motivo_cancelamento, cu.created_at
  FROM public.clube_usuarios cu
  WHERE cu.barbearia_id = p_modelo_id
    AND EXISTS (SELECT 1 FROM _map_usuarios WHERE old_id = cu.usuario_id)
    AND EXISTS (SELECT 1 FROM _map_clubes WHERE old_id = cu.clube_id);
  ...
  INSERT INTO public.promocao (id, numero_promo, imagem_promo, texto_promo, data_promo,
    created_at, testada, texto_promo_ia_2, texto_promo_ia_3, imagem_upada, barbearia_id,
    texto_promo_auxiliar, imagem_banner)
  SELECT m.new_id, p.numero_promo, p.imagem_promo, p.texto_promo, p.data_promo,
    p.created_at, p.testada, p.texto_promo_ia_2, p.texto_promo_ia_3, p.imagem_upada, v_new_id,
